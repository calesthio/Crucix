  ...
  _compactForStorage(data) {
    return {
      ...
      markets: data.markets ? {
        indexes: (data.markets.indexes || []).map(i => ({ symbol: i.symbol, price: i.price })),
        commodities: (data.markets.commodities || []).map(c => ({ symbol: c.symbol, price: c.price })),
        vix: data.markets.vix ? { value: data.markets.vix.value } : null,
      } : undefined,
