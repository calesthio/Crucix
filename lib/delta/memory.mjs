...
export class MemoryManager {
  ...
  _getAllRunsNewestFirst() {
    ...
    return candidates
      .map(run => ({ ...run, _ts: new Date(run?.timestamp || run?.data?.meta?.timestamp || 0).getTime() }))
      .filter(run => run._ts)
  ...
  getTrendSummary(windowsHours = [24, 72, 168]) {
    ...
    const summarizeWindow = (hours) => {
      ...
      return {
        ...
        urgentTempo: {
          current: newest.tg?.urgentCount || 0,
          average: avg(runs.map(run => run.data?.tg?.urgentCount || 0)),
          max: Math.max(...runs.map(run => run.data?.tg?.urgentCount || 0)),
          totalPostsCurrent: newest.tg?.posts || 0,
        },
        signals: {
          corroboratedCurrent: newest.corroboratedSignalsSummary?.total || 0,
          corroboratedAverage: avg(runs.map(run => run.data?.corroboratedSignalsSummary?.total || 0)),
          suspectCurrent: newest.suspectSignalsSummary?.total || 0,
          suspectAverage: avg(runs.map(run => run.data?.suspectSignalsSummary?.total || 0)),
          suspectPeak: Math.max(...runs.map(run => run.data?.suspectSignalsSummary?.total || 0)),
        },
        marketRegime: {
        ...
        sourceHealth: {
          degradedRuns: runs.filter(run => (run.data?.healthSummary?.degraded || 0) > 0).length,
          staleRuns: runs.filter(run => (run.data?.healthSummary?.stale || 0) > 0).length,
          failedRuns: runs.filter(run => (run.data?.healthSummary?.failed || 0) > 0).length,
          currentFailed: newest.healthSummary?.failed || 0,
          maxFailed: Math.max(...runs.map(run => run.data?.healthSummary?.failed || 0)),
        },
        recurringNews: {
        ...
        reviewPressure: {
          trackedRegionCount: newest.clusterReviewStats?.trackedRegionCount || 0,
          chronicFailureCount: newest.clusterReviewStats?.chronicFailureCount || 0,
          recentFailureCount: newest.clusterReviewStats?.recentFailureCount || 0,
        },
      };
    };
