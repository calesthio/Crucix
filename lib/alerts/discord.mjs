...
export class DiscordAlerter {
  ...
  _buildAlertEmbed(evaluation, delta, tier) {
    ...
    if (evaluation.evidence?.filter(ev => ev.url).length) {
      fields.push({
        name: 'Evidence',
        value: evaluation.evidence.filter(ev => ev.url).slice(0, 2).map(ev => `[${(ev.source || 'evidence').substring(0,20)}](${ev.url})`).join(' · '),
        inline: false,
      });
    }
  ...
  _ruleBasedEvaluation(signals, delta) {
    ...
    if (corroboratedClusters.length >= 1) {
      ...
      return { shouldAlert: true, tier: 'PRIORITY', confidence: 'HIGH', headline: topCluster.label || 'Corroborated Regional Cluster',
        reason: `A regional cluster is backed by multiple domains, which outranks raw Telegram urgency. ${topCluster.reason || ''}`.trim(), actionable: 'Review the corroborated region first and compare it to suspect-only chatter.',
        signals: corroboratedClusters.map(s => s.label || s.key).slice(0, 3), evidence: topCluster.evidence || [], crossCorrelation: `osint + hard-data (weight ${(topCluster.regionalWeight || 0).toFixed(2)})` };
    }
