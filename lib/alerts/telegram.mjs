...
const MARKET_REGIME_THRESHOLDS = {
  vix: [5, 10, 20],
  hy_spread: [5, 10, 20],
  '10y2y': [10, 20, 40],
  wti: [3, 5, 8],
...
export class TelegramAlerter {
   ...
   */
  async evaluateAndAlert(llmProvider, delta, memory, snapshot = null) {
    ...
    const lifecycleSignals = this._collectLifecycleSignals(snapshot, memory);
    allSignals.push(...lifecycleSignals.filter(s => s.phase !== 'state_only'));
    allSignals.push(...this._buildSnapshotPrioritySignals(snapshot));
  ...
  _ruleBasedEvaluation(signals, delta) {
    ...
    const corroboratedClusters = signals.filter(s => s.key?.startsWith('corroborated:'));
    const suspectClusters = signals.filter(s => s.key?.startsWith('suspect:'));
    ...
    corroboratedClusters.sort((a, b) => (b.regionalWeight || 0) - (a.regionalWeight || 0));
    suspectClusters.sort((a, b) => (b.regionalWeight || 0) - (a.regionalWeight || 0));

    if (corroboratedClusters.length >= 1) {
      const topCluster = corroboratedClusters[0];
      return {
        shouldAlert: true, tier: 'PRIORITY', confidence: 'HIGH',
        headline: topCluster.label || 'Corroborated Regional Cluster',
        reason: `A regional cluster is backed by multiple domains, which outranks raw Telegram urgency. ${topCluster.reason || ''}`.trim(),
        actionable: 'Review the corroborated region first and compare it to suspect-only chatter.',
        signals: corroboratedClusters.map(s => s.label || s.key).slice(0, 3),
        evidence: topCluster.evidence || [],
        crossCorrelation: `osint + hard-data (weight ${(topCluster.regionalWeight || 0).toFixed(2)})`,
      };
    }
    ...
    // PRIORITY: ≥5 new OSINT posts (surge in conflict reporting), but only when not outweighed by suspect-only posture
    if (osintNew.length >= 5 && suspectClusters.length < Math.max(2, corroboratedClusters.length + 1) && suspectClusters.every(s => (s.decayMultiplier || 1) > 0.5)) {
      return {
        shouldAlert: true, tier: 'PRIORITY', confidence: 'MEDIUM',
  ...
  _signalKey(signal) {
    // Market metrics are keyed by regime band so persistent elevated conditions do not
    // retrigger every sweep unless the move materially worsens into a higher band.
    if (signal?.key && MARKET_REGIME_THRESHOLDS[signal.key] && typeof signal.pctChange === 'number') {
      const regime = this._marketSignalRegime(signal);
      return `${signal.key}:${signal.direction || 'flat'}:${regime}`;
    }
  ...
  _marketSignalRegime(signal) {
    const levels = MARKET_REGIME_THRESHOLDS[signal.key];
    if (!levels) return 'raw';

    const magnitude = Math.abs(signal.pctChange || 0);
    if (magnitude >= levels[2]) return 'extreme';
    if (magnitude >= levels[1]) return 'high';
    if (magnitude >= levels[0]) return 'elevated';
    return 'normal';
  }
  ...
  _buildSnapshotPrioritySignals(snapshot) {
    if (!snapshot) return [];

    const items = [];
    for (const item of (snapshot.corroboratedSignals || []).slice(0, 4)) {
      items.push({
        key: `corroborated:${(item.signal || item.category || 'signal').toLowerCase().replace(/[^a-z0-9]+/g, '_')}`,
        label: item.signal || item.category || 'Corroborated signal',
        reason: item.reason,
        ...
        regionalWeight: item.regionalWeight || 0.5,
        evidence: item.evidence || [],
      });
    }
    for (const item of (snapshot.suspectSignals || []).slice(0, 4)) {
      items.push({
        key: `suspect:${(item.signal || item.category || 'signal').toLowerCase().replace(/[^a-z0-9]+/g, '_')}`,
        label: item.signal || item.category || 'Suspect signal',
        reason: item.reason,
        severity: item.confidence === 'low' ? 'medium' : 'low',
        direction: 'up',
        confidence: item.confidence,
        sourceHealth: item.sourceHealth,
        evidenceSource: item.evidenceSource,
        regionalWeight: (item.regionalWeight || 0.5) * (item.decayMultiplier || 1),
        decayMultiplier: item.decayMultiplier || 1,
...
- If corroborated regional clusters exist, prefer them over raw Telegram surge counts
...
Respond with ONLY valid JSON:
{
  ...
  _buildSignalContext(signals, delta) {
    ...
    const osintSignals = signals.filter(s => s.key === 'tg_urgent' || s.item?.channel || s.key?.startsWith('corroborated:') || s.key?.startsWith('suspect:'));
    const conflictSignals = signals.filter(s => ['conflict_events', 'conflict_fatalities', 'thermal_total'].includes(s.key));
  ...
  _formatTieredAlert(evaluation, delta, tier) {
    ...
    if (evaluation.evidence?.length) {
      lines.push('', `*Evidence:*`);
      for (const ev of evaluation.evidence.slice(0, 2)) {
        const label = [ev.source, ev.title].filter(Boolean).join(': ').substring(0, 120);
        if (ev.url) lines.push(`• ${escapeMd(label)}\n  ${ev.url}`);
      }
    }
