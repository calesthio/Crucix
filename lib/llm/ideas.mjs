...
function compactSweepForLLM(data, delta, previousIdeas) {
  ...
  // Crypto market
  if (data.crypto?.btc) {
    const btcPrice = data.crypto.btc.price?.toFixed(0);
    const btcChange = data.crypto.btc.change24h?.toFixed(2);
    const ethPrice = data.crypto.eth?.price?.toFixed(0);
    const dominance = data.crypto.dominance || 'N/A';
    const mktCap = data.crypto.marketCap ? `$${(data.crypto.marketCap / 1e12).toFixed(1)}T` : 'N/A';
    sections.push(`CRYPTO_MARKET: BTC $${btcPrice} (${btcChange > 0 ? '+' : ''}${btcChange}%) | ETH $${ethPrice} | BTC Dominance ${dominance}% | Market Cap ${mktCap}`);
  }
  ...
  // Internet health (BGP hijacks, outages = geopolitical signal)
  if (data.cloudflare?.radar?.hijacks?.length) {
    const hijackAlert = data.cloudflare.radar.hijacks
      .map(h => `${h.asnName || h.asn}: ${h.location}`)
      .join('; ');
    sections.push(`INTERNET_HEALTH: BGP_HIJACK_ALERT — ${hijackAlert}`);
  } else if (data.cloudflare?.health?.status === 'DEGRADED' || data.cloudflare?.health?.status === 'CRITICAL') {
    sections.push(`INTERNET_HEALTH: REGIONAL_OUTAGES — Global ${data.cloudflare.health.global_ok_pct}% OK`);
  }
  ...
  // Breaking news headlines (Reuters, WSJ, FT, Bloomberg, etc.)
  if (data.newsFeed?.length) {
    const topHeadlines = data.newsFeed.slice(0, 8).map(n => `${n.source}: ${(n.title || '').substring(0, 85)}`);
    sections.push(`BREAKING_NEWS:\n${topHeadlines.map(h => `- ${h}`).join('\n')}`);
  }
