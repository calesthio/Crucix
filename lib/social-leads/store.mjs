...
import { getSocialLeadAcquisitionCapabilities, listAcquisitionTiers, planSocialLeadAcquisition } from './acquisition.mjs';
...
export function normalizeSocialLeadInput(input = {}, options = {}) {
  ...
  const attachments = Array.isArray(input.attachments)
    ...
    : [];
  const acquisitionPlan = planSocialLeadAcquisition(input, options.capabilities);
  ...
  if (!rawText && quotedThreadText.length === 0 && !acquisitionPlan.allowUrlOnlyPlaceholder) {
  ...
  return {
    ...
    attachments,
    captureMethod,
    acquisitionTier: acquisitionPlan.resolvedTier,
    requestedAcquisitionTier: acquisitionPlan.requestedTier,
    acquisitionPlan,
    observedAt,
  };
}
...
export function createSocialLead(input = {}, options = {}) {
  const capabilities = options.capabilities || getSocialLeadAcquisitionCapabilities(options.env);
  const normalized = normalizeSocialLeadInput(input, { capabilities });
  ...
  return {
    ...
    leadId,
    status: normalized.acquisitionPlan.needsManualEvidence ? 'captured-with-manual-gap' : 'captured',
    source: {
      ...
      requestedAcquisitionTier: normalized.requestedAcquisitionTier,
      acquisitionDetail: {
        usedLogin: normalized.acquisitionTier === 'formal-api',
        usedBrowser: normalized.acquisitionTier === 'browser-assisted',
        usedApi: normalized.acquisitionTier === 'formal-api',
        usedPasteFallback: normalized.acquisitionTier === 'manual-text',
        retrievalStatus: normalized.acquisitionPlan.retrievalStatus,
        degradation: normalized.acquisitionPlan.degradation,
        nextAction: normalized.acquisitionPlan.nextAction,
        capabilities: normalized.acquisitionPlan.capabilities,
      },
    },
    rawEvidence: {
    ...
    workflow: {
      acquisition: normalized.acquisitionPlan,
    },
  };
}

export function createSocialLeadStore({ rootDir, maxLeads = DEFAULT_MAX_LEADS, env = process.env } = {}) {
  ...
  function intake(input = {}, options = {}) {
    const state = load();
    const lead = createSocialLead(input, { ...options, env });
  ...
  function buildContract(state = load(), { limit = 10 } = {}) {
    ...
    return {
      ...
      capabilities: {
        ...
        acceptedAcquisitionTiers: listAcquisitionTiers(),
        acquisitionCapabilities: getSocialLeadAcquisitionCapabilities(env),
      },
      notes: [
        'This social-leads contract preserves operator-submitted X evidence and acquisition metadata before automated retrieval, claim extraction, or verification fanout exists.',
        'When a requested retrieval tier is unavailable, Crucix degrades gracefully and records the remaining manual evidence gap instead of silently pretending retrieval succeeded.',
      ],
    };
  }
