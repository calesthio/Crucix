...
function buildRuntimeBucketContract(items = []) {
  const contract = new Map();
  for (const item of items) {
    const bucket = item?.runtimeBucket;
    if (!bucket || !Array.isArray(bucket.attributionAliases) || !bucket.attributionAliases.length) continue;
    const normalizedAliases = bucket.attributionAliases
      .map(alias => normalizeAttributionName(alias))
      .filter(Boolean);
    const entry = {
      sourceId: item.id || null,
      runtimeSource: item.name,
      kind: bucket.kind || 'single-publisher',
      rationale: bucket.rationale || null,
      attributionAliases: normalizedAliases,
      declaredAliases: Array.from(new Set(bucket.attributionAliases.filter(Boolean))),
    };
    for (const alias of normalizedAliases) {
      contract.set(alias, entry);
    }
  }
  return contract;
}

function summarizeRuntimeBucketDrift(items = [], observedRuntimeGroups = new Map(), runtimeBucketContract = new Map()) {
  const drifts = [];

  for (const item of items) {
    const runtimeToken = normalizeAttributionName(item?.name);
    if (!runtimeToken) continue;
    const observed = observedRuntimeGroups.get(runtimeToken) || null;
    const contract = runtimeBucketContract.get(runtimeToken) || null;
    const runtimeBucket = item?.runtimeBucket || null;
    const declaredAliases = Array.isArray(runtimeBucket?.attributionAliases) ? runtimeBucket.attributionAliases.filter(Boolean) : [];
    const declaredAliasTokens = new Set(declaredAliases.map(alias => normalizeAttributionName(alias)).filter(Boolean));
    const observedPublishers = observed
      ? Array.from(observed.sourceNames.entries())
          .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))
          .map(([name, count]) => ({ name, count, token: normalizeAttributionName(name) || 'unknown' }))
      : [];
    const observedPublisherTokens = new Set(observedPublishers.map(entry => entry.token));
    const unexpectedObservedAliases = observedPublishers.filter(entry => entry.token && !declaredAliasTokens.has(entry.token));
    const missingObservedAliases = declaredAliases.filter(alias => !observedPublisherTokens.has(normalizeAttributionName(alias)));
    const observedPublisherCount = observedPublishers.length;
    const clusterCount = observed?.clusterIds?.size || 0;
    ...
    if (runtimeBucket?.kind === 'single-publisher' && observedPublisherCount > 1) {
      driftKind = 'single-publisher-mismatch';
      severity = observedPublisherCount >= 3 ? 'high' : 'medium';
      summary = `${item.name} is declared single-publisher but is currently attributed to ${observedPublisherCount} publisher labels`;
      recommendation = 'review source contract and decide whether the runtime bucket should become expected-multi-publisher';
    } else if (runtimeBucket?.kind === 'expected-multi-publisher' && unexpectedObservedAliases.length > 0) {
      driftKind = 'missing-aggregator-alias';
      severity = unexpectedObservedAliases.length >= 2 ? 'medium' : 'low';
      summary = `${item.name} observed ${unexpectedObservedAliases.length} publisher label${unexpectedObservedAliases.length === 1 ? '' : 's'} outside its declared aggregator aliases`;
      recommendation = 'review whether new upstream publisher aliases should be added to the source-registry runtime-bucket contract';
    }
    ...
    drifts.push({
      ...
      runtimeBucket: runtimeBucket ? {
        kind: runtimeBucket.kind || 'single-publisher',
        rationale: runtimeBucket.rationale || null,
        attributionAliases: declaredAliases,
      } : null,
      contractKind: contract?.kind || runtimeBucket?.kind || 'single-publisher',
      driftKind,
      severity,
      summary,
      recommendation,
      ...
      observedPublishers: observedPublishers.map(({ name, count }) => ({ name, count })),
      unexpectedObservedAliases: unexpectedObservedAliases.map(({ name, count }) => ({ name, count })),
      missingObservedAliases,
    });
  }
  ...
  return {
    ...
    singlePublisherMismatchCount: drifts.filter(item => item.driftKind === 'single-publisher-mismatch').length,
    missingAggregatorAliasCount: drifts.filter(item => item.driftKind === 'missing-aggregator-alias').length,
    highSeverityCount: drifts.filter(item => item.severity === 'high').length,
    items: drifts.sort((a, b) => {
      const severityRank = { high: 3, medium: 2, low: 1, none: 0 };
      return (severityRank[b.severity] || 0) - (severityRank[a.severity] || 0)
        || b.observedPublisherCount - a.observedPublisherCount
        || b.clusterCount - a.clusterCount
        || a.runtimeSource.localeCompare(b.runtimeSource);
    }),
  };
}

function classifyRuntimeBucket(runtimeToken, runtimeSource, sourceNames = new Map(), runtimeBucketContract = new Map()) {
  ...
  const looksLikeExpectedMultiPublisher =
    contract?.kind === 'expected-multi-publisher' ||
    (!contract && !containsRuntimeNamedAlias && uniqueTokens.size >= 3);

  if (looksLikeExpectedMultiPublisher) {
    return {
      kind: 'expected-multi-publisher',
      aliasLike: false,
      reason: contract?.kind === 'expected-multi-publisher'
        ? (contract.rationale || 'runtime bucket is declared as expected multi-publisher in source registry')
...
function buildAttributionUncertaintyDiagnostics(items = [], snapshot = {}) {
  ...
  for (const cluster of clusters) {
    ...
    for (const entry of entries) {
      ...
      if (!observedRuntimeGroups.has(runtimeToken)) observedRuntimeGroups.set(runtimeToken, { runtimeSource: runtimeName, sourceNames: new Map(), clusterIds: new Set() });
      observedRuntimeGroups.get(runtimeToken).sourceNames.set(sourceName, (observedRuntimeGroups.get(runtimeToken).sourceNames.get(sourceName) || 0) + (Number(entry?.count) || 1));
      observedRuntimeGroups.get(runtimeToken).clusterIds.add(cluster.id || 'unknown');
    ...
    for (const [runtimeToken, info] of runtimeGroups.entries()) {
      ...
      const classification = classifyRuntimeBucket(runtimeToken, info.runtimeSource, info.sourceNames, runtimeBucketContract);
    ...
    const duplicateRuntimeSources = Array.from(runtimeGroups.entries())
      ...
      .filter(item => classifyRuntimeBucket(item.runtimeToken, item.runtimeSource, item.sourceNames, runtimeBucketContract).aliasLike)
  ...
  const runtimeBucketDrift = summarizeRuntimeBucketDrift(items, observedRuntimeGroups, runtimeBucketContract);
  ...
  return {
    ...
    summary: {
      ...
      runtimeBucketDriftCount: runtimeBucketDrift.totalDriftCount,
      singlePublisherMismatchCount: runtimeBucketDrift.singlePublisherMismatchCount,
      missingAggregatorAliasCount: runtimeBucketDrift.missingAggregatorAliasCount,
    },
    aliasCollisions,
    expectedMultiPublisherBuckets,
    ambiguousMappings,
...
function summarizeSourcePerformanceHistory(snapshot = {}) {
  ...
  return {
    ...
    snapshots: snapshots.slice(0, 8).map(item => ({
      ...
      runtimeBucketDrift: item.runtimeBucketDrift || null,
    ...
    deltaViews: deltaViews.slice(0, 6).map(item => ({
      ...
      topRuntimeBucketShifts: Array.isArray(item.topRuntimeBucketShifts) ? item.topRuntimeBucketShifts : [],
    })),
  };
}
...
export function buildSourceOpsSurface({ rootDir, snapshot = null } = {}) {
  ...
  const inventoryItems = sources.map(source => {
    ...
    return {
      ...
      runtimeBucket: source.runtimeBucket || null,
  ...
  return {
    contract: profile ? {
      ...
      lifecycleEvaluationSchemaPath: profile?.lifecycleEvaluationSchemaPath || null,
      lifecycleBatchSchemaPath: profile?.lifecycleBatchSchemaPath || null,
