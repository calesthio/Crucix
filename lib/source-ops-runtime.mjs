...
function buildAttributionUncertaintyDiagnostics(items = [], snapshot = {}) {
  ...
  for (const cluster of clusters) {
    ...
    for (const [runtimeToken, info] of runtimeGroups.entries()) {
      ...
      if (!aliasSummary.has(runtimeToken)) aliasSummary.set(runtimeToken, {
        runtimeSource: info.runtimeSource,
        sourceNames: new Map(),
        clusterIds: new Set(),
      });
      const target = aliasSummary.get(runtimeToken);
    ...
    const duplicateRuntimeSources = Array.from(runtimeGroups.values())
      .filter(info => Array.from(info.sourceNames.keys()).length > 1)
      .map(info => info.runtimeSource);
