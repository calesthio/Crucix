...
import { summarizeSourceGradingRubric, summarizeSourceScorecard } from './source-scorecard.mjs';
...
export function loadSourceOpsWorkspace(rootDir) {
  ...
  return { profile, registry, needs, rubric, exampleScorecard };
}

export function buildSourceOpsSurface({ rootDir, snapshot = null } = {}) {
  const { profile, registry, needs, rubric, exampleScorecard } = loadSourceOpsWorkspace(rootDir);
