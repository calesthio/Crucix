...
function summarizeSourcePerformance(items = [], snapshot = {}) {
  ...
  const measuredItems = items.map(item => {
    const counts = counters.get(item.name) || { feedItems: 0, suspectSignals: 0, corroboratedSignals: 0, degradationSignals: 0, citedByTippingPoints: 0 };
    ...
    const attentionScore = counts.corroboratedSignals * 3 + counts.suspectSignals * 2 + counts.degradationSignals * 2 + counts.citedByTippingPoints * 2 + Math.min(counts.feedItems, 3) + livePenalty;
    ...
    return {
      ...
      measurementBasis: {
        ...
        directClusterAttribution: false,
      },
    };
  ...
  return {
    ...
    items: measuredItems,
    measurementNotes: {
      directClusterAttribution: 'not yet available per source, so current performance uses feed, signal, tipping-point, and live-state proxies',
