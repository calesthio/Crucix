...
- Add first-class operator queue actions such as ack, snooze, and jump-to-artifact links, so the review queue becomes directly actionable without forcing operators back into separate debug endpoints.
- Capture sampled high-pressure queue fixtures or queue snapshots from real runs, so prioritization tuning can be validated against realistic operator cases instead of only synthetic ranking tests and the current live runtime state.
