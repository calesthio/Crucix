...
- Add endpoint-level contract coverage for reasoning metadata on `/api/analysis` and `/api/brief/news` so source-role context cannot silently drop from live operator surfaces.
