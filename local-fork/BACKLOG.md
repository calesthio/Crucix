...
The operator settings, layout overhaul, runtime reliability, source-ops console, LLM or agent operations, review workflow, deployment hardening, contract-expansion, and critical-alerting recommendations from 2026-04-25 were integrated into `local-fork/plan.json` as Epics 27 through 35.
...
- Add panel-level render budget telemetry and optional virtualization for wallboard mode so long OSINT/news streams and large review queues do not silently degrade browser responsiveness as the dashboard keeps growing. Added to backlog on 2026-04-25 after cycle 091.
- Add a saved panel-layout reset plus named operator presets so the new per-panel collapse, pin, priority, and size controls can be recovered from bad workspace states and switched intentionally between analyst, narrow-screen, and wallboard profiles. Added to backlog on 2026-04-25 after cycle 092.
- Add operator-editable preset authoring, clone/reset controls, and import-export for workspace presets so layouts stop being code-defined bundles and can be tuned safely from the UI. Added to backlog on 2026-04-25 after cycle 093.
